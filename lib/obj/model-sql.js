...
ModelSQL.prototype = {
  ...
  _getInsertFileds: function () {
    ...
    for(var key in self._fieldInfo){
      if(self._fieldInfo[key]["auto_increment"]){
        continue;
      }
  ...
  fieldInfo:function(fieldInfo){
    ...
    for(var key in fieldInfo){
      if(fieldInfo[key].key && fieldInfo[key].key.toUpperCase() == "PRIMARY KEY"){
        self._priKey = key;
        break;
      }
    }
    self._priKey = self._priKey || "";
  },
    ...
    for (var i = 0; i < tmp.length; i++) {
      ...
      for(var key in fields){
        ...
        if(util.type(value) == "undefined"){
          ...
          value = "null";
        }if(fields[key].type.toUpperCase() == "JSON" && typeof value == "object"){
